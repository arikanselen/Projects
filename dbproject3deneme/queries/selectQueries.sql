...
where s.issn = 'i1' and s.yearr=2023 and s.semester = 2 and s.courseCode = c.courseCode
...
where i.ssn not in (select s.issn
                    from sectionn s
                    where s.yearr=2023 and s.semester = 2)

-- 6.   List the students taking course C in a given year Y and semester S. (students taking COMP2222 in Spring 2023)

select s.studentname
from enrollment e, student s
where e.yearr=2023 and e.semester = 2 and e.courseCode = 'comp2222' and e.sssn = s.ssn;


-- 7.   List the students taking a particular section S (Note that, particular section means that all the compound key  fields of section is fixed, course C, instructor I, year Y, semester SE, section id ID). (students taking COMP2222.1  of Emine Ekin in Spring 2023)

select s.studentname
from enrollment e, student s, instructor i
where e.yearr=2023 and e.semester = 2 and e.courseCode = 'comp2222' and e.sssn = s.ssn and 
    i.iname = 'Emine Ekin' and i.ssn= e.issn and e.sectionId = 1
...
-- 9.   Given a student S, semester SE, year Y, display timetable in the form [coursecode, section id, day, hour] like COMP2222,1,DK,113,M,5
...
where E.sssn = 's1' and E.semester = 2 and E.yearr=2023
...
from enrollment E
where E.courseCode = 'COMP2222' and E.yearr = 2023 and E.semester = 2;

-- 12.  Display all scores of a student S of a course C in the form [examname, score].
select STE.examname, sum(GPQ.pointsEarned)  
from studenttakingexam STE NATURAL JOIN gradesperquestion GPQ
where STE.courseCode="COMP2222" and STE.sssn="s1"
GROUP by STE.examname, STE.yearr,STE.semester
--last two options in group by is to list repeating courses exam scores seperately.
...
select sssn, qno, pointsEarned
from gradesperquestion
where courseCode="COMP2222" and examname="Midterm1" and semester=2 and yearr=2023;
...
where (sectionn.yearr = 2023 and sectionn.semester=2 ) or (sectionn.yearr is null and sectionn.semester is null)
GROUP BY I.ssn
